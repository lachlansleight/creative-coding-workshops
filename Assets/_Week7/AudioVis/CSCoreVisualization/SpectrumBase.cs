...
namespace WinformsVisualization.Visualization
{
    public class SpectrumBase : INotifyPropertyChanged
    {
        ...
        {
            get { return _spectrumResolution; }
            set
            {
                ...
                RaisePropertyChanged("SpectrumResolution");
            }
        }
        ...
        public float[] GetSpectrumPoints(double maxValue, float[] fftBuffer)
        {
            var points = CalculateSpectrumPoints(maxValue, fftBuffer);
            var output = new float[points.Length];
            for (var i = 0; i < points.Length; i++) output[i] = (float) points[i].Value;
            return output;
        }
